        ...
        <ul style={{ marginTop: 16, paddingLeft: 18 }}>
          {events.map((event) => (
            <li key={event.id} style={{ marginBottom: 8 }}>
              <Link href={`/_archive-youth-league/events/${event.id}`}>{event.name}</Link>
            </li>
          ))}
        </ul>
      )}
      <p style={{ marginTop: 16 }}>
        Need request analysis? Visit <Link href="/_archive-youth-league/request-analysis">Request Analysis</Link>.
      </p>
    </div>
  );
}
