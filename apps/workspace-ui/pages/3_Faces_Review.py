...
with st.expander("📊 Similarity Scores Guide", expanded=False):
    ...
    with col4:
        _render_similarity_card(
            ...
            ["≥ 0.82 soft threshold (continue track)", "≥ 0.75 hard threshold (force split)"],
        )
    ...
    quality_df = {
        "Badge": ["Q: 85%+", "Q: 60-84%", "Q: < 60%", "ID: 75%+", "ID: 60-74%", "ID: < 60%"],
        "Meaning": [
...
def _select_episode() -> str:
    ...
    with st.sidebar:
        ...
        if current and is_locked:
            ...
            if st.button("🔓 Change Episode", key="unlock_episode_selector", use_container_width=True):
        ...
        with tracked_tab:
            ...
            if options:
                ...
                col1, col2 = st.columns(2)
                with col1:
                    if st.button("✓ Load", key="facebank_load_tracked", use_container_width=True, type="primary"):
                        st.session_state["episode_selector_unlocked"] = False
                        helpers.set_ep_id(selection)
                with col2:
                    if st.button("✗ Cancel", key="cancel_episode_change", use_container_width=True):
                        st.session_state["episode_selector_unlocked"] = False
                        st.rerun()
            else:
...
def _episode_header(ep_id: str) -> Dict[str, Any] | None:
    ...
    with action_cols[2]:
        if st.button("🔄 Refresh Values", key="facebank_refresh_similarity_button", help="Recompute all similarity scores"):
    ...
    for row_start in range(0, len(cast_cards), cols_per_row):
        ...
        for idx, card in enumerate(row_members):
            with cols[idx]:
                ...
                if person_id and episode_clusters:
                    if st.button("View", key=f"view_cast_{cast_id}_{person_id}", use_container_width=True):
...
def _render_unassigned_cluster_card(
    ...
    with st.container(border=True):
            ...
            if total_pages > 1:
                ...
                with col_left:
                    if st.button("◀", key=f"prev_{cluster_id}", disabled=current_page == 0):
                        ...
                        st.rerun()
                with col_center:
                    st.caption(f"Showing tracks {current_page * max_visible + 1}-{min((current_page + 1) * max_visible, num_tracks)} of {num_tracks}")
                with col_right:
                    if st.button("▶", key=f"next_{cluster_id}", disabled=current_page >= total_pages - 1):
                    ...
                    with cols[idx]:
                        ...
                        if thumb_url:
                            ...
                            if "s3.amazonaws.com/" in thumb_url:
                                ...
                                thumb_markup = helpers.thumb_html(s3_key, alt=f"Track {track_id}", hide_if_missing=False)
                            else:
                                thumb_markup = helpers.thumb_html(thumb_url, alt=f"Track {track_id}", hide_if_missing=False)
                            st.markdown(thumb_markup, unsafe_allow_html=True)
        ...
        if suggested_cast_id and suggested_cast_name:
            ...
            with sugg_col2:
                ...
                with st.form(key=f"quick_assign_form_{cluster_id}"):
                    submit_quick_assign = st.form_submit_button("✓", help="Accept suggestion and assign", type="primary", use_container_width=True)
        ...
        if assign_choice == "Existing cast member":
                ...
                with st.form(key=f"assign_form_unassigned_{cluster_id}"):
                    ...
                    if submit_assign:
                        with st.spinner("Assigning cluster..."):
                            ...
                            people_resp = _safe_api_get(f"/shows/{show_id}/people")
                            people = people_resp.get("people", []) if people_resp else []
                            target_person = next((p for p in people if p.get("cast_id") == selected_cast_id), None)
            ...
            if st.button("Create person", key=f"create_person_btn_{cluster_id}", disabled=not new_name):
...
def _render_auto_person_card(
    ...
    if episode_clusters:
        ...
        if suggestions_resp:
            ...
            first_cluster = episode_clusters[0].split(":")[-1] if episode_clusters else None
            ...
            if assign_choice == "Existing cast member":
                if cast_options:
                    ...
                    with st.form(key=f"assign_form_{person_id}"):
                        ...
                        if suggested_cast_id and suggested_cast_id == selected_cast_id and suggested_distance is not None:
                    ...
                    with row_cols[idx]:
                        ...
                        with btn_cols[0]:
                            if st.button("View", key=f"view_cluster_{person_id}_{cluster_id}"):
                                ...
                                st.rerun()
                        with btn_cols[1]:
                            if st.button("Delete", key=f"delete_cluster_{person_id}_{cluster_id}", type="secondary"):
                                resp = _api_delete(f"/episodes/{ep_id}/identities/{cluster_id}")
                                if resp:
                                    st.success(f"Deleted cluster {cluster_id}")
                                    st.rerun()
...
def _bulk_assign_to_new_person(
    ...
    try:
        ...
        existing = next(
            (p for p in people if p.get("name", "").lower() == new_name.lower()),
            None
        )
...
def _render_people_view(
    ...
    if unassigned_clusters:
        ...
        with header_col1:
            ...
            st.caption(f"Clusters detected but not yet assigned to cast members. Review and assign manually.")
        with header_col2:
            if st.button("💾 Save Progress", key=f"save_progress_{ep_id}", help="Save all current assignments"):
...
def _render_person_clusters(
    ...
    sort_option = st.selectbox(
        "Sort by:",
        ["Track Similarity (Low to High)", "Track Similarity (High to Low)", 
         "Frame Count (Low to High)", "Frame Count (High to Low)",
         "Average Frame Similarity (Low to High)", "Average Frame Similarity (High to Low)",
         "Track ID (Low to High)", "Track ID (High to Low)"],
    ...
    if sort_option == "Track Similarity (Low to High)":
        ...
        all_tracks.sort(key=lambda t: t.get("similarity") if t.get("similarity") is not None else -999.0, reverse=True)
        ...
        all_tracks.sort(key=lambda t: int(t.get("track_id", "0").replace("track_", "")) if isinstance(t.get("track_id"), str) else 0)
    elif sort_option == "Track ID (High to Low)":
        all_tracks.sort(key=lambda t: int(t.get("track_id", "0").replace("track_", "")) if isinstance(t.get("track_id"), str) else 0, reverse=True)
    ...
    for track in all_tracks:
        ...
        with st.container(border=True):
            ...
            st.markdown(f"**Track {track_num}** {badge_html} · Cluster `{cluster_id}` · {len(frames)} frames", unsafe_allow_html=True)
            ...
            with col1:
                if track_id_int is not None and st.button("View all frames", key=f"view_track_frames_{person_id}_{track_id_int}"):
                    _set_view("track", person_id=person_id, identity_id=cluster_id, track_id=track_id_int)
            with col2:
                if track_id_int is not None:
                    ...
                    if move_targets and st.button("Move track", key=f"move_track_{person_id}_{track_id_int}"):
                        ...
                        st.rerun()
            with col3:
                if track_id_int is not None and st.button("Delete", key=f"delete_track_{person_id}_{track_id_int}", type="secondary"):
...
if st.button(
    ...
    if selected_strategy == "auto":
        ...
        if error:
            ...
            cleared = next((s.get('cleared_count', 0) for s in steps if s.get('step') == 'clear_assignments'), 0)
            centroids = next((s.get('centroids_count', 0) for s in steps if s.get('step') == 'compute_centroids'), 0)
            merged = next((s.get('merged_count', 0) for s in steps if s.get('step') == 'group_within_episode'), 0)
            suggestions = next((s.get('suggestions_count', 0) for s in steps if s.get('step') == 'group_across_episodes'), 0)
