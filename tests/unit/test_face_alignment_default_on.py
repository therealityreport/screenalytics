...
def test_face_alignment_config_is_default_on() -> None:
    import yaml

    cfg_path = Path("config/pipeline/face_alignment.yaml")
    data = yaml.safe_load(cfg_path.read_text(encoding="utf-8")) or {}
    face_alignment = data.get("face_alignment") or {}
...
def test_faces_embed_writes_alignment_artifacts_by_default(tmp_path, monkeypatch) -> None:
    ...
    ep_id = "test-face-alignment-default-on"
    run_id = "runABC"
    ensure_dirs(ep_id)
...
def test_faces_embed_does_not_write_alignment_artifacts_when_disabled(tmp_path, monkeypatch) -> None:
    data_root = tmp_path / "data"
    monkeypatch.setenv("SCREENALYTICS_DATA_ROOT", str(data_root))

    ep_id = "test-face-alignment-disabled"
    run_id = "runABC"
    ensure_dirs(ep_id)
    ...
    # Simulate rollback by disabling face_alignment.enabled.
    monkeypatch.setattr(
        episode_run,
        "_load_alignment_config",
        lambda: {"face_alignment": {"enabled": False}},
    )
