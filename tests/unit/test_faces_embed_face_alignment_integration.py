...
def _write_single_track(ep_id: str) -> None:
...
def test_faces_embed_uses_aligned_crop_when_face_alignment_enabled(tmp_path, monkeypatch) -> None:
    ...
    # Enable face alignment for this run (avoid reading repo config in unit tests).
    monkeypatch.setattr(
        episode_run,
    ...
    # Provide a deterministic embedder and frame decoder.
    ...
    # Patch FAN alignment to avoid requiring the face-alignment dependency in CI.
