...
def test_delete_episode_removes_local_data(tmp_path, monkeypatch):
    ...
    resp = client.post(f"/episodes/{ep_id}/delete", json={"include_s3": False})
...
def test_delete_all_invokes_s3_prefix_cleanup(tmp_path, monkeypatch):
    ...
    assert len(deleted_prefixes) >= len(ep_ids) * 5
