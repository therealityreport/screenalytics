...
def test_grouping_service_writes_new_dict_format():
    ...
    func_match = re.search(
        r'def compute_cluster_centroids\(.*?\):.*?return output',
        content,
        re.DOTALL
    )
    ...
    assert 'centroids[cluster_id] = centroid_entry' in func_code, \
        "Should assign centroids by cluster_id key"

    # Check that it includes tracks
    assert '"tracks": tracks_formatted' in func_code, \
...
def test_grouping_service_handles_both_formats():
    ...
    assert 'isinstance(centroids, list)' in content, \
        "Should check for list format"
    assert 'isinstance(centroids, dict)' in content, \
        "Should check for dict format"
    assert 'centroids_list = [' in content, \
        "Should convert dict to list for processing"
...
def test_track_reps_converts_legacy_format():
    ...
    assert 'if isinstance(centroids, list):' in content, \
        "Should detect legacy list format"
    assert 'identity_tracks_map' in content, \
        "Should load identities to get track_ids"
    assert '"tracks": tracks' in content or '"tracks": identity_tracks_map' in content, \
...
def test_build_cluster_track_reps_guards_incomplete_data():
    ...
    assert 'def build_cluster_track_reps(' in content, \
        "build_cluster_track_reps function should exist"
    assert 'if not cluster_data:' in content, \
        "Should check if cluster_data exists"
    assert '"tracks": []' in content, \
        "Should return empty tracks for missing clusters"
