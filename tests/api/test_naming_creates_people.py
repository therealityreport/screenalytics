...
def test_persist_identity_name_creates_people_record():
    ...
    assert (
        "from apps.api.services.people import PeopleService" in content
    ), "Should import PeopleService"
    assert (
        "people_service = PeopleService()" in content
    ...
    assert (
        'cluster_id_with_prefix = f"{ep_id}:{identity_id}"' in content
...
def test_naming_doesnt_fail_on_people_service_error():
    ...
    assert (
        "Don't fail the naming operation" in content
        or "Failed to create/update People" in content
...
def test_person_record_includes_cluster_id():
    ...
    assert (
        "cluster_ids=[cluster_id_with_prefix]" in content
    ), "Should pass cluster_ids when creating person"
    assert (
        'cluster_ids = existing_person.get("cluster_ids", [])' in content
