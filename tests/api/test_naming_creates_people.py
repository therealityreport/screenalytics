...
def test_persist_identity_name_creates_people_record():
    ...
    assert 'from apps.api.services.people import PeopleService' in content, \
        "Should import PeopleService"
    assert 'people_service = PeopleService()' in content, \
        "Should instantiate PeopleService"

    # Check that it creates or updates person
    assert 'people_service.create_person(' in content, \
        "Should create new person"
    assert 'people_service.add_cluster_to_person(' in content, \
        "Should add cluster to existing person"

    # Check that it handles name matching (case-insensitive)
    assert 'existing_person = next(' in content, \
        "Should search for existing person by name"
    assert '.lower()' in content, \
        "Should do case-insensitive name matching"

    # Check that cluster_id includes episode prefix
    assert 'cluster_id_with_prefix = f"{ep_id}:{identity_id}"' in content, \
        "Should prefix cluster_id with episode"
...
def test_naming_doesnt_fail_on_people_service_error():
    ...
    assert 'except Exception as exc:' in content, \
        "Should catch exceptions from PeopleService"
    assert 'LOGGER.warning' in content, \
        "Should log warning on failure"
    assert "Don't fail the naming operation" in content or \
           "Failed to create/update People" in content, \
...
def test_person_record_includes_cluster_id():
    ...
    assert 'cluster_ids=[cluster_id_with_prefix]' in content, \
        "Should pass cluster_ids when creating person"
    assert 'cluster_ids = existing_person.get("cluster_ids", [])' in content, \
        "Should check existing cluster_ids"
