...
def test_fetch_track_media_uses_cursor_and_returns_tuple() -> None:
    ...
    fetch_func = next(
        node for node in module.body if isinstance(node, ast.FunctionDef) and node.name == "_fetch_track_media"
    )
    ...
    has_tuple_return = any(
        isinstance(node.value, ast.Tuple) for node in ast.walk(fetch_func) if isinstance(node, ast.Return)
    )
