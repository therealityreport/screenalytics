...
def test_cluster_view_shows_placeholder_for_missing_crops():
    ...
    assert missing_crop_count == 0, f"Found {missing_crop_count} 'Missing crop' captions, should show placeholder instead"
...
def test_set_view_no_rerun_in_callback():
    ...
    assert 'st.rerun()' not in set_view_code, \
        ...
        if '_set_view(' in line and 'lambda' not in line and 'on_click' not in line:
            ...
            if i + 1 < len(lines):
                ...
                if next_line == 'return':
                    ...
                    assert 'st.rerun()' in next_line or i + 2 < len(lines) and 'st.rerun()' in lines[i + 2], \
