...
def main():
    ...
    for person in people:
        ...
        if name.lower() in ["kim richards", "kim"]:
            ...
            print(f"Found: {person['person_id']} - {person.get('name')} - cast_id:{person.get('cast_id')} - clusters:{len(person.get('cluster_ids', []))}")
    ...
    for entry in kim_entries:
        if entry.get("cast_id"):
            if canonical:
                print(f"Warning: Multiple entries with cast_id! Using {canonical['person_id']}")
            else:
                canonical = entry
    ...
    if not canonical:
        ...
        print(f"No cast_id found, using first entry as canonical: {canonical['person_id']}")

    print(f"\nMerging {len(to_merge)} entries into {canonical['person_id']}:")

    for source in to_merge:
        print(f"  Merging {source['person_id']} ({source.get('name')}) -> {canonical['person_id']}")
        ...
        if result:
            print(f"    ✓ Success - {len(result.get('cluster_ids', []))} total clusters")
        else:
            print(f"    ✗ Failed")
    ...
    if final:
        ...
        print(f"  Cluster IDs: {json.dumps(sorted(set(final.get('cluster_ids', []))), indent=2)}")
