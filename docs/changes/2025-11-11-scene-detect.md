...
## Summary
- Integrated PySceneDetect as the default hard-cut detector (with the legacy HSV histogram as a fallback), emitting `scene_detect:*` SSE events, resetting ByteTrack/StrongSORT at each cut, and forcing `SCENE_WARMUP_DETS` consecutive detections after every reset.
- Plumbed `scene_detector`, `scene_threshold`, `scene_min_len`, and `scene_warmup_dets` through the CLI, job service/router payloads, and the Episode Detail UI (advanced toggle + per-run badge) with env defaults: `SCENE_DETECTOR=pyscenedetect`, `SCENE_THRESHOLD=27.0`, `SCENE_MIN_LEN=12`, `SCENE_WARMUP_DETS=3`.
