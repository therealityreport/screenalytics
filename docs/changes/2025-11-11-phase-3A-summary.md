...
- Streamlit uploader that creates episodes, PUTs the video (local or S3/MinIO), mirrors bytes to `data/videos/{ep_id}/episode.mp4`, and optionally calls the detect/track stub.
- `POST /jobs/detect_track` shells out to `tools/episode_run.py --stub`, counting detections/tracks written under `data/manifests/{ep_id}/`.
...
```
In the UI, enter Show/Season/Episode, upload a small `.mp4`, check “Run detect/track (stub)”, and submit.

Verify:
- UI reports the `ep_id` and "Upload successful".
- `data/videos/{ep_id}/episode.mp4` exists.
- `data/manifests/{ep_id}/detections.jsonl` and `tracks.jsonl` are populated by the stub.

## Acceptance checklist
- [x] `POST /episodes` returns deterministic `ep_id`
- [x] `POST /episodes/{ep_id}/assets` returns presigned PUT (or local path)
- [x] Streamlit flow handles upload + stub trigger
