...
|----------|-------|-------------|
| `NEXT_PUBLIC_API_BASE` | `https://api.<screenalytics-domain>` | Backend API URL |
...
2. **Test SSE streaming**
   ```bash
   curl -N https://api.<domain>/episodes/test-ep/events
   ```

3. **Run a small pipeline job**
   - Upload a short video via the UI
   - Trigger detect_track
   - Verify job completes and results appear
